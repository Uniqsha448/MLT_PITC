...
# List of all MLT coaches — add/remove names here
COACH_LIST = [
    "Select your name...",
    "Coach Admin (All Access)",
    "Aaliyah Johnson", "Brandon Williams", "Camille Davis",
    "DeShawn Thompson", "Elena Martinez", "Fatima Okonkwo",
    "Gregory Chen", "Hannah Robinson", "Isaiah Washington",
    "Jasmine Patel", "Kevin Brown", "Latoya Harris",
    "Marcus Rivera", "Natasha Scott", "Omar Abdullah",
    "Priya Nair", "Quinton Jackson", "Renee Foster",
]
...
def app_to_features(app):
    ...
    return {
        ...
        'Summer_Internship':           1 if any(x in title.lower() for x in ['summer','intern']) else 0,
        'Position_Type':               infer_position_type(title),
        'Seniority_Level':             infer_seniority(title),
        'Title_Word_Count':            len(title.split()) if title else 1,
        'Industry_Subsector':          industry if industry and industry not in ['','nan','None'] else 'Other',
        'Company_Size_Bucket':         app.get('company_size','Mid (1K-10K)'),
        'Is_Fortune500':               1 if company.strip() in FORTUNE_500 else 0,
    }
...
if st.session_state.coach_name is None:
    ...
    with col_c:
        ...
        <div class="login-card">
          <h2>👤 Coach Login</h2>
          <p>Select your name to access your applicants.<br>Each coach sees only their own caseload.</p>
        </div>
        """, unsafe_allow_html=True)

        selected = st.selectbox("Select your name", COACH_LIST, key="login_select")
        ...
        if st.button("▶ Enter Dashboard", type="primary", use_container_width=True):
            if selected == "Select your name...":
                ...
                st.session_state.coach_name = selected
                saved_apps, _ = load_from_file(selected)
...
IS_ADMIN = CURRENT_COACH == "Coach Admin (All Access)"
